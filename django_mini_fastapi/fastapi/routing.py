...
from ..base import Response, Request, JSONResponse
from . import params
...
from ..route import BaseRoute, Router
...
def run_endpoint_function(*, dependant: Dependant, values: Dict[str, Any]) -> Any:
...
def get_request_handler(
    ...
    def app(request: Request) -> Response:
        try:
            ...
            if body_field:
                if is_body_form:
                    combined_formdata = QueryDict("", mutable=True)
                    combined_formdata.update(request.POST)
                    combined_formdata.update(request.FILES)
                    body = combined_formdata
                else:
        ...
        if errors:
            ...
            # FIXME: background task is no supprted for now
            response_args.pop("background")

            response = actual_response_class(response_data, **response_args)

            apply_sub_response_metadata(response, sub_response)
...
class APIRoute(BaseRoute):
    def __init__(
        ...
        *,
        ...
        self.unique_id = generate_operation_id_for_path(
            name=self.name, path=self.path, method=list(methods)[0]
        )
        ...
        self.app = self.route_handler = self.get_route_handler()
